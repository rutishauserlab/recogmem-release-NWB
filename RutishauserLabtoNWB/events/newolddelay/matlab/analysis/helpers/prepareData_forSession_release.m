...
function [params, basedirData_forSession,basedirEvents_forSession,brainArea] = prepareData_forSession_demo( basepath, NOsession_toUse, paramsIn )
...
[~, stimuliRecog, newOldRecogLabels, recogResponses, responsesEventfile, recallResponses, recallEventfile, newOldRecall, eventsRecog, categoryMappingRecog, ~, ...
    RTRecog_raw, RTRecog_vsQ] = NOloadDataOfBlock_release( NOsession_toUse, 1,basepath ); %1 mode recog

%load stimuli --learning
if NOsession_toUse.EXPERIMENTIDLearn>0    %if this session has a learning block
    [stimuliLearn, ~, ~, ~, ~, ~, ~, ~, eventsLearn, categoryMappingLearn] = NOloadDataOfBlock_release( NOsession_toUse, 2, basepath ); %2 mode recog
else
    stimuliLearn=[];
    eventsLearn=[];
end
...
%% load data files
%== paths
pathSpikeData = 'sorted/';
pathEventData = 'events/';
basedirData_forSession   = [basepath pathSpikeData NOsession_toUse.session '/' NOsession_toUse.taskDescr '/'];
basedirEvents_forSession =[basepath pathEventData NOsession_toUse.session '/' NOsession_toUse.taskDescr '/'];

% load brain area mapping (channel -> brain area)
fNameBrainArea = [basedirEvents_forSession 'brainArea.mat'];
